# Vigia -- a price-watching agent, built FROM the plow-hermes-agent base.
#
...
# reporter refuses to guess -- the base's agent-index service parks
...
LABEL org.opencontainers.image.source="https://github.com/emanuellcoelho/vigia-hermes-agent" \
      org.opencontainers.image.licenses="MIT" \
      org.opencontainers.image.description="Drop a product link; it watches the price and tells you when it drops, over iMessage."
...
COPY LICENSE NOTICE /usr/share/doc/vigia/
...
COPY --chmod=0755 image/cont-init.d/10-vigia-timezone /etc/cont-init.d/10-vigia-timezone

# The instance directory the skills read and onboarding writes: watched items,
# price history, config. Empty until first boot; owned by the agent's uid.
RUN install -d -o 10000 -g 10000 -m 0700 /var/lib/hermes/vigia
